...
void paintStrokeFromXYPtoXYP(float fromX, float fromY, float toX, float toY) {
    goToStartPose();
    smoothStrokeElastic(fromX, fromY, fromX, fromY, threeBrushVec56, 90, false, true, false);
    smoothStrokeElastic(fromX, fromY, toX, toY, threeBrushVec56, 90, true, false, false);
    smoothStrokeElastic(toX, toY, toX, toY, threeBrushVec56, 90, false, false, true); 
}   

void smoothStrokeElastic(float startX, float startY, float endX, float endY, PVector vec56, float brushAngle, boolean strokeOnly, boolean putBrushDown, boolean pickBrushUp) {
   ...
   if (strokeOnly) {
     smoothStrokeFloat(increments, getElasticRobotX(startX,startY), getElasticRobotY(startY), getElasticRobotZforRobotY(startY), getElasticRobotX(endX,startY), getElasticRobotY(endY), getElasticRobotZforRobotY(endY), vec56, brushAngle, theta6);
   } else if (putBrushDown) {
     smoothStrokeFloat(increments, getElasticRobotX(startX,startY), getElasticRobotY(startY)+yAdjustment, getElasticRobotZforRobotY(startY)+zAdjustment, getElasticRobotX(endX,startY), getElasticRobotY(endY), getElasticRobotZforRobotY(endY), vec56, brushAngle, theta6);
   } else if (pickBrushUp) {
     smoothStrokeFloat(increments, getElasticRobotX(startX,startY), getElasticRobotY(startY), getElasticRobotZforRobotY(startY), getElasticRobotX(endX,startY), getElasticRobotY(endY)+yAdjustment, getElasticRobotZforRobotY(endY)+zAdjustment, vec56, brushAngle, theta6);
   }
}
...
void moveBrush(float startX, float startY, float endX, float endY, PVector vec56, float brushAngle) {
   ...
   smoothStrokeFloat(increments, getRobotX(startX), getRobotY(startY)-zAdjustment, getRobotZforRobotY(startY)+zAdjustment, getRobotX(endX), getRobotY(endY)-zAdjustment, getRobotZforRobotY(endY)+zAdjustment, vec56, brushAngle, theta6);
}
...
void smoothStrokeFloat(int increments, float startX, float startY, float startZ, float endX, float endY, float endZ, PVector vec56, float brushAngle , float theta6 ) {
  ...
  PVector j6 = new PVector(startX,//+getElasticArcXAdjustment(10, startX,startY),
                           startY,//+getElasticArcYZAdjustment(5, startX, 5),
                           startZ//+getElasticArcYZAdjustment(9, startX, -9)
                          );
  ...
  for (int i=1; i<=steps; i++) {
    ...
    float nextY = startY+(stepY*i);//+getElasticArcYZAdjustment(5, nextX, 5);
          nextX = startX+(stepX*i);//+getElasticArcXAdjustment(10, nextX, nextY);
    float nextZ = startZ+(stepZ*i);//+getElasticArcYZAdjustment(9, nextX, -9);
    System.out.println(nextX+" : "+nextY+" : "+nextZ);
    j6 = new PVector(nextX, nextY , nextZ);
    setIK(j6, vec56, brushAngle, theta6);
    while(!isAllConverge){delay(1);}
  }
  j6 = new PVector(endX,//+getElasticArcXAdjustment(10, endX, endY),
                   endY,//+getElasticArcYZAdjustment(5, endX, 5),
                   endZ//+getElasticArcYZAdjustment(9, endX, -9)
                          );
  setIK(j6, vec56, brushAngle, theta6);
  while(!isAllConverge){delay(10);}
}
