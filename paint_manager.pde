...
String[] colorWellMapping = {  "#fac88c", "#dcaa64", "#aa5028", "#8c000a", "#aaaaaa","#505050", "#fafafa", "#050505", "#xxxxxx", "#xxxxxx", "#xxxxxx", "#xxxxxx" };
int[]        brushMapping = {          3,         3,         3,         3,         3,         3,         3,        1,        3,         3,         3,         3 };
...
PVector getBrushVector() {
  return threeBrushVec56; //centerVec56;
}
...
void paintPercentage(String index, int strokesPerColor, int startStroke, int endStroke) {
  ...
  while (true && currentStroke <= endStroke) {
    ...
    if (hasHits(brushstroke)) {
      ...
      //record time of strokes in global variable - save to elasticsearch db or file
...
*/
      ...
      smoothStrokeElastic(nextX, nextY, nextX, nextY,getBrushVector(), getBrushAngle(brushNumber), false, true, false);
      lastX = nextX;
      lastY = nextY;
      ...
      for (int i=0; i<numberOfPoints; i++) {
        ...
        smoothStrokeElastic(lastX, lastY, nextX, nextY,getBrushVector(), getBrushAngle(brushNumber), true, false, false);
        lastX = nextX;
        lastY = nextY;
      }
      smoothStrokeElastic(lastX, lastY, nextX, nextY,getBrushVector(), getBrushAngle(brushNumber), false, false, true);
      goToStrokePost(getBrushAngle(brushNumber));
...
void paint(String index, int startStroke) {
  ...
  while (true) {
    ...
    if (hasHits(brushstroke)) {
      ...
      //println("nx "+nextX+" : "+nextY+" ");
      moveBrush(0.5, 0.5, nextX, nextY,getBrushVector(), defaultBrushAngle);     
      smoothStrokeElastic(nextX, nextY, nextX, nextY,getBrushVector(), defaultBrushAngle, true, false, false);
      lastX = nextX;
      lastY = nextY;
      int numberOfPoints = path.size();
      println("numberPoints "+numberOfPoints+" : "+nextY+" ");
      for (int i=0; i<numberOfPoints; i++) {
        ...
        nextX = point.getFloat(0)/xWidth;
        nextY = point.getFloat(1)/yHeight;
        smoothStrokeElastic(lastX, lastY, nextX, nextY,getBrushVector(), defaultBrushAngle, true, false, false);
        lastX = nextX;
        lastY = nextY;
      }
      moveBrush(lastX, lastY, lastX, lastY,getBrushVector(), defaultBrushAngle);  
